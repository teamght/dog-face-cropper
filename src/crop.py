...
class DogFaceCrop():
  ...
  def find_center_brain(self, img, left_eye, right_eye, nose):
    ...
    #show_with_points(img, [center_eyes, center_muzzle, center_brain])
    return center_brain
  ...
  def crop_with_padding(self, img, x0, y0, x1, y1):
    ...
    #show_with_points(table, [])
    return table[y0+table_origin[0]:y1+table_origin[0], x0+table_origin[1]:x1+table_origin[1]]

  def crop(self, img):
    detection, shape = get_detection_and_shape(self.detector, self.predictor, img)
    #show_with_points(img, shape)
